...
(function(){
  ...
  if (!grid || !titleN) {
    console.error('[Calendrier] #calGrid ou #calTitle introuvable(s). Vérifie tes IDs dans le HTML.');
    return;
  }
  ...
  // -------- Chargement ICS (avec fallback de chemins) --------
  async function fetchIcsWithFallback() {
    ...
    for (const url of candidates) {
      try {
        ...
        console.warn('[Calendrier] fetch fail', url, e);
      }
    }
  ...
  // Lock scroll (réutilise ta lightbox)
