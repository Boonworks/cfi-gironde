...
// reCAPTCHA v3 (⚠️ remplace par tes NOUVELLES clés)
$RECAPTCHA_SITE_KEY = '6Ldf9KQrAAAAAIbradKR6SjGQxckNMtvsQXj7qxI';

// SMTP (remplis selon ton hébergeur)
// Exemples courants :
// - OVH: host 'ssl0.ovh.net', port 587 (TLS) ou 465 (SSL)
// - IONOS: 'smtp.ionos.fr', 587 TLS
// - Infomaniak: 'mail.infomaniak.com', 587 TLS
// - Google Workspace: 'smtp.gmail.com', 587 TLS (avec mot de passe d'application)
$SMTP = [
  'host'   => 'mail01.lwspanel.com',
  'port'   => 587,            // 587 (TLS) ou 465 (SSL)
  'secure' => 'tls',          // 'tls' ou 'ssl'
  'user'   => 'no-reply@cfi-gironde.fr', // compte SMTP
  'from'   => 'no-reply@cfi-gironde.fr', // adresse d’expéditeur sur VOTRE domaine
  'pass'   => $secrets['smtp_pass'],
];
/* ============================================= */

// CSRF token si absent
  ...
  if (!empty($_POST['website'] ?? '')) {
    ...
    if (!$errors) {
      ...
      try {
        ...
        // From doit être sur ton domaine
        ...
        //$mail->SMTPDebug = 2; // 2 = infos serveur
